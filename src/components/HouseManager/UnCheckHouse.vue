...
<section>
    ...
    <template>
        <el-table :data="list" highlight-current-row v-loading="listLoading" border style="width: 100%;top: 0px;
            ...
            <el-table-column prop="renovation" label="装修" width="80">
            </el-table-column>
...
export default {
    computed: mapGetters({
        list: 'uncheckHouseList',
        page: 'uncheckHousePage'
    }),
    created() {
        let params = {
        ...
        this.$store.dispatch('uncheckHouseList', params)
    },
    ...
    methods: {
        ...
        formatFeature(feature) {
            ...
            //审核通过
            reviewPass() {
                this.houseDetailVisible = false;
                let params = {
                    houseId: this.house.id,
                    type: 0
                }
                this.$store.dispatch('reviewHouse', params)
            },

            reviewReject() {

                this.infoFormVisible = false;
                this.$prompt('感谢您提交宝贵的房源信息,【内容编辑部分】,期待您的再次提交。', '被拒理由', {
